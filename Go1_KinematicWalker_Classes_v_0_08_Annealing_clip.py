...
class Agent(nn.Module):
    def __init__(self, envs, actor_layer_size, critic_layer_size):
        ...
        self.actor_mean = nn.Sequential(
            layer_init(nn.Linear(np.array(envs.single_observation_space.shape).prod(), actor_layer_size)),
            nn.SiLU(),
            ...
            layer_init(nn.Linear(actor_layer_size, actor_layer_size)),
            nn.SiLU(),
            layer_init(nn.Linear(actor_layer_size, np.prod(envs.single_action_space.shape)), std=0.01),
        )
...
if __name__ == "__main__":
    ...
    if test_and_not_run:
        ...
        plot_tracking = True
        plot_tracking = False
        walker_envs_values = create_walkers(n=8, render_first=first_env_block_render, plot_tracking=plot_tracking, xml_file=xml_file, 
